...
<<<<<<< HEAD
    // Poll for new messages every 5s while page is open
...
<<<<<<< HEAD
    ...
    const handleStatusChange = async (newStatus: PlanStatus) => {
        ...
        try {
            ...
            if (newStatus === "confirmed") {
                showToast("🎉 予定が確定しました！");
                const friendIds = await getFriendIds(plan.creatorId);
                await Promise.all(
                    friendIds.map((fId) =>
                        addNotification({
                            id: generateId(),
    ...
    const handleDelete = async () => {
        ...
        const ok = window.confirm(
            "この予定を削除しますか？参加者にも見えなくなります。この操作は取り消せません。",
        );
        if (!ok) return;
        setDeleting(true);
        try {
            // Notify other participants the plan is gone so the chat doesn't
            // silently disappear on them.
            const others = plan.participantIds.filter((uid) => uid !== currentUserId);
            await Promise.all(
                others.map((uid) =>
                    addNotification({
                        id: generateId(),
                        ...
                        type: "plan_join",
                        title: "予定が削除されました",
                        body: `${creator?.displayName ?? "主催者"} の ${park?.name ?? "予定"} (${timeSlotLabel(plan.timeSlot)}) は削除されました`,
                        isRead: false,
                        createdAt: new Date().toISOString(),
                    }),
                ),
            ...
            const target = plan.parkId ? `/parks/${plan.parkId}` : "/";
            router.push(target);
        } catch (err) {
            console.error(err);
            showToast("削除に失敗しました");
    ...
    const handleLeave = async () => {
        if (!currentUserId || isCreator || leaving) return;
        if (!plan.participantIds.includes(currentUserId)) return;
        const ok = window.confirm("この予定への参加を取り消しますか？");
        if (!ok) return;
        setLeaving(true);
        try {
            const updated: Plan = {
                ...plan,
                participantIds: plan.participantIds.filter((uid) => uid !== currentUserId),
            };
            await updatePlan(updated);
            await logEvent({
                type: "participants_changed",
            ...
            await addMessage({
                ...
                planId,
                senderId: "system",
                text: `${me?.displayName ?? "参加者"} が参加を取り消しました`,
                createdAt: new Date().toISOString(),
                isSystem: true,
            });
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!currentUserId || !newMsg.trim() || isArchived || sending) return;
        setSending(true);
...
<<<<<<< HEAD
                    <div className="mt-3 flex flex-col gap-2">
                        <button
                            ...
                            className={`w-full py-2 rounded-xl text-sm font-semibold transition-colors ${plan.status === "confirmed"
=======
...
<<<<<<< HEAD
                        <button
                            ...
                            disabled={deleting}
                            className="w-full py-2 rounded-xl text-sm font-semibold bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-colors disabled:opacity-40"
                        >
                            {deleting ? "削除中…" : "🗑️ 予定を削除する"}
                        </button>
=======
>>>>>>> 16bb157cb2a9d74dca5345d0be0ea2409118efde
                    </div>
...
<<<<<<< HEAD
                {isParticipating && !isCreator && !isArchived && (
                    <button
                        onClick={handleLeave}
                        disabled={leaving}
                        className="mt-4 w-full bg-white text-red-600 border border-red-200 py-2.5 rounded-xl font-semibold text-sm hover:bg-red-50 transition-colors disabled:opacity-40"
                    >
                        {leaving ? "取り消し中…" : "参加を取り消す"}
                    </button>
                )}
=======
>>>>>>> 16bb157cb2a9d74dca5345d0be0ea2409118efde
